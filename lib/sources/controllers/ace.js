...
zz.ide.controllers.Ace.prototype.openFileHandler_ = function( e ){
    ...
    }else if( zz.environment.services.Environment.getInstance( )

            .getRootController( )
            .getOpenFile( ) ){

        var data = goog.json.parse( e.messageData[ 0 ][ 4 ] );
        var fileType = data[ 0 ][ 0 ].slice( goog.array.lastIndexOf( data[ 0 ][ 0 ], '.' ) + 1 );
        var name = data[ 0 ][ 0 ].slice( goog.array.lastIndexOf( data[ 0 ][ 0 ], '/' ) + 1 );
        var aceMode;
        var model = this.getModel( );
