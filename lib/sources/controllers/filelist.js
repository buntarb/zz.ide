...
zz.ide.controllers.Filelist.prototype.openFileHandler_ = function( e ){
    ...
    if( error.length ){
        ...
        if( zz.environment.services.Environment.getInstance( )
            ...
            var fileType = oldName.slice( goog.array.lastIndexOf( oldName, '.' ) );
            oldName = oldName.slice( 0, goog.array.lastIndexOf( oldName, '.' ) );
            var name = oldName + zz.ide.enums.Const.COPY_SUFFIX + fileType;
