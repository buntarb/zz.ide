...
zz.ide.controllers.Filelist.prototype.getChildrenHandler_ = function( e ){
    ...
    if( datarow.type === zz.net.enums.MessageType.ERROR ){
        ...
        var data = goog.json.unsafeParse(datarow.data)[ 0 ][ 6 ];
        model = this.getModel( );
...
zz.ide.controllers.Filelist.prototype.saveFileHandler_ = function( e ){
    ...
    var content = goog.json.parse( e.messageData[ 0 ][ 4 ])[ 0 ][ 2 ];
    var model = this.getModel();
    var loop = true;
    var addFile = false;
    ...
    if ( model.firstDatarow( ) ){
        ...
        while ( loop ){
            ...
            if ( model.currentDatarow( ).name !== fileName ){
    ...
    if( addFile ){
        ...
        // TODO: Blinker: divide create new file and save old
        if( !content && !zz.environment.services.Environment.getInstance( )
            ...
            zz.environment.services.Environment
                ...
                .getInstance( )
                .getRootController( )
                .openModal( true, false, 200, 100, 'File with the same name is already exist' );
    ...
    if( datarow.type === zz.net.enums.MessageType.ERROR ){
        ...
        if ( model.firstDatarow( ) ){
            ...
            while ( loop ){
                ...
                if ( model.currentDatarow( ).path !== goog.json.unsafeParse( e.messageData[ 0 ][ 4 ] )[ 0 ][ 0 ] ){

                    loop = model.nextDatarow( );

                }else{

                    model.currentDatarow( ).path = goog.json.unsafeParse( e.messageData[ 0 ][ 4 ] )[ 0 ][ 1 ];
                    model.currentDatarow( ).name =

                        model.currentDatarow( ).path.
                        slice(  goog.array.lastIndexOf( model.currentDatarow( ).path, '/' ) + 1 );
                    loop = false;
                    console.log( model.currentDatarow( ) );
                }
            }
        }
    }
