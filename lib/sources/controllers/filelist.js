        ...
        if( getChildrenMessageModel.firstDatarow( ).stderr
            ...
            if( item = getChildrenMessageModel.firstDatarow( ).children.firstDatarow( ) ){
                ...
                do{
                    ...
                    // '/?file' was added for get file icon from folders, not folder icon.
                    var icon = zz.ide.services.ConstantsConverter

                        .getInstance( )
                        .getIconFromRoute( this.router_.getFragment( ) + '/?file=' );
                    var className = zz.ide.services.ConstantsConverter

                        .getInstance( )
                        .getClassFromRoute( this.router_.getFragment( ) );
                    model.createLast( [ name, item.type, item.path, icon, className ] );
