...
zz.ide.controllers.Header.prototype.actionHandler_ = function( e ){
    ...
    if( this.getView( ).isActionShowNavigation( e ) ){
        ...
        if( this.layout_.getModel( ).lastDatarow( ).popup.length
            ...
            && this.layout_.getModel( ).lastDatarow( ).popup.lastDatarow().id === 'services' ){
            ...
            var servicesController = new zz.ide.controllers.Services( servicesModel );
...
zz.ide.controllers.Header.prototype.searchQueryHandler_ = function( e ){
    ...
    if( e.message.datafield === this.getModel( ).datafield.search ){
        ...
        var query = e.message.new_value;

        this.layout_.closePopup( );
        if( query !== '' ){

            zz.ide.services.SearchApi.getInstance( ).sendQueryAutocomplite( query );
...
zz.ide.controllers.Header.prototype.searchResponseHandler_ = function( e ){
    ...
    var searchDataMessageModel = new zz.ide.models.JSDocQuery( undefined, e.getDataJson( ) );
    var searchData = goog.json.parse( searchDataMessageModel.lastDatarow( ).result );
