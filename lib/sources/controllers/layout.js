...
zz.ide.controllers.Layout.prototype.actionHandler_ = function( e ){
    ...
    if( this.getView( ).isActionAddBtn( e ) ){
       ...
       this.openModalAddFile( );
        ...
        this.compile( zz.ide.enums.Command.COMPILE_APP );

    }else if( this.getView( ).isActionCompileSoy( e ) ){

        this.compile( zz.ide.enums.Command.COMPILE_SOY );

    }else if( this.getView( ).isActionCompileStyles( e ) ){

        this.compile( zz.ide.enums.Command.COMPILE_STYLES );

    }else if( this.getView( ).isActionCompileModels( e ) ){

        this.compile( zz.ide.enums.Command.COMPILE_MODELS );

    }else if( this.getView( ).isActionCalculatingDeps( e ) ){

        this.compile( zz.ide.enums.Command.CALCDEPS );
...
zz.ide.controllers.Layout.prototype.compileHandler_ = function( e ){
    ...
    switch( e.type ){
        ...
        case zz.ide.enums.Command.COMPILE_APP:
            ...
            // TODO: Make it with one function
...
zz.ide.controllers.Layout.prototype.routeChangedHandler_ = function( opt_e ){
    ...
    if( goog.dom.pattern.matchStringOrRegex( /\/models/, route ) ){
        ...
        // TODO: Make it with one function
        this.getModel( ).lastDatarow( ).colorGreen = false;
        this.getModel( ).lastDatarow( ).colorOrange = false;
        this.getModel( ).lastDatarow( ).colorBlue = true;

    }else if( goog.dom.pattern.matchStringOrRegex( /\/templates/, route ) ) {

        // TODO: Make it with one function
        this.getModel().lastDatarow().colorBlue = false;
        this.getModel().lastDatarow().colorGreen = false;
        this.getModel().lastDatarow().colorOrange = true;
        ...
        this.getModel( ).lastDatarow( ).colorBlue = false;
        this.getModel( ).lastDatarow( ).colorOrange = false;
        this.getModel( ).lastDatarow( ).colorGreen = true;
 ...
 */
zz.ide.controllers.Layout.prototype.compile = function( command ){
    ...
    switch( command ){
        ...
        case zz.ide.enums.Command.CALCDEPS:
            ...
            if( !this.getModel( ).lastDatarow( ).calculatingDeps ){
                ...
                zz.ide.services.ClientApi.getInstance( ).calculatingDeps( );
            }
            this.getModel( ).lastDatarow( ).compiling = true;
