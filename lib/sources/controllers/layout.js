...
zz.ide.controllers.Layout.prototype.actionHandler_ = function( e ){
    ...
    }else if( e.elements[ 0 ].getAttribute( zz.ide.enums.DataAction.BACK_BTN ) ){

        // TODO: Move it to separate method
        this.getRouter( ).back( );

    }else if( e.elements[ 0 ].getAttribute( zz.ide.enums.DataAction.FORWARD_BTN ) ){

        // TODO: Move it to separate method
        this.getRouter( ).forward( );
    }
...
zz.ide.controllers.Layout.prototype.routeChangedHandler_ = function( opt_e ){
    ...
    if( opt_e ){
        ...
        route = this.getRouter( ).getFragment( );
    }
    ...
    if( this.getRouter( ).getHistoryStack( ).length ){
    ...
    if( this.getRouter( ).getHistoryStackForward( ).length ){
