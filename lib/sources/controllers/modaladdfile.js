...
zz.ide.controllers.ModalAddFile.prototype.actionHandler_ = function( e ){
    ...
    if( e.elements[ 0 ].getAttribute( zz.ide.enums.DataAction.CLOSE_MODAL ) ){
        ...
        var filelistModel = zz.environment.services.Environment.getInstance( ).getRootController( )
        ...
        if ( filelistModel.firstDatarow( ) ){
            ...
            while ( loop ){
                ...
                if ( filelistModel.currentDatarow( ).name !== e.model.name ){
        ...
        if( addFile ){
            ...
            zz.environment.services.Environment.getInstance( ).getRootController( ).getModel( ).lastDatarow( )

                .modal.lastDatarow( ).title = 'File with the same name ' + e.model.name + ' is already exist';
        }
    }
    e.stopPropagation( );
};