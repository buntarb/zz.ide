// Copyright 2016 Artem Lytvynov <buntarb@gmail.com>. All Rights Reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS-IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/**
 * @license Apache-2.0
 * @copyright Artem Lytvynov <buntarb@gmail.com>
 */

goog.provide( 'zz.ide.controllers.Navigation' );

goog.require( 'goog.dom' );

goog.require( 'zz.controllers.FEBase' );
goog.require( 'zz.ide.views.Navigation' );
goog.require( 'zz.ide.models.Navigation' );
goog.require( 'zz.models.enums.EventType' );
goog.require( 'zz.ide.enums.CssClass' );
goog.require( 'zz.ide.enums.Const' );

/**
 * Navigation controller.
 * @param {zz.ide.models.Navigation} model
 * @param opt_dom
 * @constructor
 * @extends {zz.controllers.FEBase}
 */
zz.ide.controllers.Navigation = function( model, opt_dom ){

    /**
     * Navigation view.
     * @type {zz.ide.views.Navigation}
     */
    var view = zz.ide.views.Navigation.getInstance( );

    goog.base( this, model, view, opt_dom );

    //this.getHandler( ).listen(
    //
    //    this,
    //    goog.ui.Component.EventType.ACTION,
    //    this.actionHandler_,
    //    false
    //);
    //this.getHandler( ).listen(
    //
    //    this.getModel( ),
    //    zz.models.enums.EventType.DATAROW_CREATE,
    //    this.datarowCreatedHandler_,
    //    false
    //);
    //this.getHandler( ).listen(
    //
    //    this.getModel( ),
    //    zz.models.enums.EventType.DATAROW_UPDATE,
    //    this.getView( ).datarowUpdatedHandler,
    //    false
    //);
};

goog.inherits( zz.ide.controllers.Navigation, zz.controllers.FEBase );
//
///**
// * Action event handler.
// * @private
// */
//zz.ide.controllers.Navigation.prototype.actionHandler_ = function( e ){
//
//    console.log( "handleAction", e );
//    if( e.item.model.type === zz.ide.enums.Const.FOLDER ){
//
//        var model = e.item.model.children;
//        if( model.length ){
//
//            while( model.deleteLast( ) ){}
//            e.item.model.isFolderOpen = false;
//
//        }else{
//
//            model.createLast(['id' + e.item.model.getUid(), 'folder']);
//            model.createLast(['file', 'file']);
//            e.item.model.isFolderOpen = true;
//        }
//
//    }
//    if( e.item.model.type === zz.ide.enums.Const.FOLDER || e.item.model.type === zz.ide.enums.Const.FILE){
//
//        var contextModel = e.item.model.context;
//
//        if( !contextModel.length ){
//
//            contextModel.createLast( );
//            var contextController = new zz.ide.controllers.Context( contextModel );
//            this.addFieldController(
//
//                e.item.model,
//                this.getModel( ).datafield.context,
//                contextController );
//        }
//    }
//    e.stopPropagation( );
//};
//
///**
// * Datarow created event handler.
// * @private
// */
//zz.ide.controllers.Navigation.prototype.datarowCreatedHandler_ = function( e ){
//
//    if( e.message.datarow.type === zz.ide.enums.Const.FOLDER
//        && e.message.datarow.children
//        && !e.message.datarow.children.length ){
//
//        var controller = new zz.ide.controllers.Navigation( e.message.datarow.children );
//        this.addFieldController(
//
//            e.message.datarow,
//            this.getModel( ).datafield.children,
//            controller );
//    }
//    e.stopPropagation( );
//};