...
goog.require( 'zz.ide.models.Node' );
goog.require( 'zz.ide.controllers.Node' );
...
zz.ide.controllers.Tree.prototype.actionHandler_ = function( e ){
    ...
    if( e.item.model.type === zz.ide.enums.Const.FOLDER ){
        ...
        if( model && model.length ){
            ...
            goog.dom.classlist.remove( e.item.view, zz.ide.enums.CssClass.FOLDER_OPEN );

        }else if( model ){

            model.createLast(['id' + e.item.model.getUid(), 'folder']);
            model.createLast(['file', 'file']);
            goog.dom.classlist.add( e.item.view, zz.ide.enums.CssClass.FOLDER_OPEN );
        }
    }
    e.stopPropagation( );
};
...
zz.ide.controllers.Tree.prototype.datarowCreatedHandler_ = function( e ){
    ...
    var nodeModel = new zz.ide.models.Node( );
    nodeModel.createLast(

        [
            e.message.datarow.title,
            e.message.datarow.type,
            e.message.datarow.path,
            e.message.datarow.content,
            e.message.datarow.count,
            false
        ]
    );
    var nodeController = new zz.ide.controllers.Node( nodeModel );
    console.log( nodeModel );
    this.addFieldController(

        e.message.datarow,
        this.getModel( ).datafield.node,
        nodeController );
    e.stopPropagation( );
};