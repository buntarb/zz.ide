 ...
 * @constructor
 */
zz.ide.services.FtService = function( ){
     ...
     * @private
     */
    this.FILE_DELIM_REG_EXP_ = new RegExp( goog.string.regExpEscape( this.ft_.CONST.PATH_DELIMITER ), 'g' );
    ...
    commandListenerMap[ zz.ide.enums.Command.REMOVE_FILE ]  = { binder: this.generalBinder_, fun: this.removeFile_ };
    commandListenerMap[ zz.ide.enums.Command.COPY_FILE ]    = { binder: this.generalBinder_, fun: this.copyFile_ };
    commandListenerMap[ zz.ide.enums.Command.RENAME_FILE ]  = { binder: this.generalBinder_, fun: this.renameFile_ };
    ...
    this.wsc_.addSupportedCommand( zz.ide.enums.Command.IDK_HELP );
    this.wsc_.addSupportedCommand( zz.ide.enums.Command.COMPILE_APP );
 ...
 * @param {string} command is a shell command of OS
...
zz.ide.services.FtService.prototype.executeHandler_ = function( treeDatarow, command, this_ ){
    ...
    var executeResponse = this_.ft_.execute( command ).toString();
    treeDatarow.content = executeResponse;
};

/**
 * Rename specified file in datarow.path to datarow.name.
 * @param {zz.ide.models.FilesTreeDatarow} treeDatarow
 * @param {?Object|undefined} this_ is a context
 * @private
 */
zz.ide.services.FtService.prototype.renameFile_ = function( treeDatarow, this_ ){
    ...
    var pathTo = this_.normalizePath_( treeDatarow.name );

    var isOk = this_.ft_.copyFile( pathFrom, pathTo );
    if( isOk ){

        this_.ft_.removeFile( pathFrom );
    }
};

/**
 * Copy specified file in datarow.path to datarow.name.
 * @param {zz.ide.models.FilesTreeDatarow} treeDatarow
 * @param {?Object|undefined} this_ is a context
 * @private
 */
zz.ide.services.FtService.prototype.copyFile_ = function( treeDatarow, this_ ){
    ...
    var pathFrom = this_.normalizePath_( treeDatarow.path );
    var pathTo = this_.normalizePath_( treeDatarow.name );
    this_.ft_.copyFile( pathFrom, pathTo );
};
...
zz.ide.services.FtService.prototype.idkCommandBinder_ = function( fun ){
    ...
    var handler = function( input ){
        ...
        if( idkCommand ){
            ...
            this.wsc_.sendErrorMessage( data.messageSource, zz.net.enums.MessageType.ERROR, 'idk command not found' );
        }
    };
...
var ftService = new zz.ide.services.FtService( );
