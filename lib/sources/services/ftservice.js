 ...
 * @param {string} url
 * @param {Object} opt_idk
 * @constructor
 */
zz.ide.services.FtService = function( url, opt_idk ){

    var idk = opt_idk? opt_idk: require( 'imazzine-developer-kit' );
...
zz.ide.services.FtService.prototype.executeHandler_ = function( treeDatarow, command, this_ ){
    ...
    var executeResponse = this_.ft_.execute( command );
...
zz.ide.services.FtService.prototype.renameFile_ = function( treeDatarow, this_ ){
    ...
    this_.ft_.saveFile( pathTo, this_.ft_.openFile( pathFrom ) );

    if( this_.ft_.isFileExist( pathTo ) ){

        treeDatarow.error = this_.ft_.removeFile( pathFrom );

        if( '' === treeDatarow.error && this_.ft_.isFileExist( pathFrom ) ){

            throw new Error( 'Cannot remove [' + pathFrom + '] while renaming' );
        }

    }else{

        throw new Error( 'Cannot rename [' + pathFrom + '] to [' + pathTo + ']' );
    }
...
zz.ide.services.FtService.prototype.copyFile_ = function( treeDatarow, this_ ){
    ...
    if( !this_.ft_.isFileExist( pathTo ) ){
        ...
        throw new Error( 'Cannot copy [' + pathFrom + '] to [' + pathTo + ']' );
    }
...
zz.ide.services.FtService.prototype.removeFile_ = function( treeDatarow, this_ ){
    ...
    treeDatarow.error = this_.ft_.removeFile( path );

    if( '' === treeDatarow.error && this_.ft_.isFileExist( path ) ){
