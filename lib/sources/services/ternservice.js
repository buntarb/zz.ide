...
zz.ide.services.TernService.prototype.requestTernHandler_ = function( input ){
    ...
    try{
        ...
        var query = goog.json.unsafeParse( inputDatarow.query );
        type = query[ 'type' ];

        var documentRequest = { };
        documentRequest[ 'query' ] = query;
        documentRequest[ 'files' ] = goog.json.unsafeParse( inputDatarow.files );
        documentRequest[ 'timeout' ] = inputDatarow.timeout;
        ...
        } else if( outDataset && type ) {

            try {

                outDataset.createLast( [ type, '', ( '' + e ) ] );
