...
zz.ide.services.TernService = function( url, opt_protocol, opt_ternServer ){
    ...
    // console.log( 'this.ternSrv_ = ' + this.ternSrv_ );
    // for( var p in this.ternSrv_ ){
    //
    //     'p = ' + p;
    // }
...
zz.ide.services.TernService.prototype.defaultQueryTern_ = function( inDatarow, outDataset, sendFun,
    ...
    this_.ternSrv_.request( documentRequest, function( err, resp ) {
        ...
        if ( err ) {
            ...
            // throw err;
            outDatarow[ datarowSchema.stderror.index ] = err;

        } else {

            if( goog.array.contains( convertToJSON, '*' ) ){ // Out datarow contains only two fields - stderror and response
                ...
                if( payloadFieldIndex < 0 ){
                    ...
                    // throw err;
                    outDatarow[ datarowSchema.stderror.index ] = 'Error: payload field (for response) not found';
                } else {

                    outDatarow[ payloadFieldIndex ] = goog.json.serialize( resp );
                    outDatarow[ datarowSchema.stderror.index ] = '';
                }
...
var ternService = new zz.ide.services.TernService( 'ws://localhost:7777' );